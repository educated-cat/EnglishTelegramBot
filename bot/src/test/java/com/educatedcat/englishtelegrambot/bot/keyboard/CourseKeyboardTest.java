...
class CourseKeyboardTest extends AbstractTest {
	@Autowired
	private CourseKeyboard courseKeyboard;
	...
	@Test
	void verifyKeyboard() {
		List<List<InlineKeyboardButton>> keyboard = courseKeyboard.getKeyboard();
	...
	private boolean keyboardCorrect(List<List<InlineKeyboardButton>> keyboard,
		...
		try {
			...
			if (inlineKeyboardButtons.size() == 1 && keyboard.indexOf(inlineKeyboardButtons) == keyboard.size() - 1) {
				return callback.button() == MenuButtonType.START;
			}
